...
class TextEncoder_New(nn.Module):
    def __init__(self, pretrained_dir, text_encoder='base'):
        """
        txt_encoder: base / large
        """
        super(TextEncoder_New, self).__init__()

        assert text_encoder in ['bert_base', 'bert_large', 'roberta_base', 'roberta_large', 'deberta_base']
        ...
        elif text_encoder in ['deberta_base']:
            tokenizer = DebertaTokenizer
            model = DebertaModel
            self.tokenizer = tokenizer.from_pretrained(pretrained_dir+'/deberta_base/')
            self.model = model.from_pretrained(pretrained_dir+'/deberta_base/')
        else:
            tokenizer = RobertaTokenizer
            model = RobertaModel
